...
/*    function reOpenFunc(){
   ...
   function pdfOpenFunc(){
        window.open("${pageContext.request.contextPath}/account/slip/report.do?method=findReport&slip_no="+slipNo, "window", "width=1600, height=1000");
   }
      ...
      $("#slipGrid").jqGrid({
         ...
         root : "slipList"
           ...
           colModel: [
                    ...
                    {name :"expenseReport",width :250 , align :"center",editable:true},
         ...
         onSelectRow: function(id) {
           ...
           beforeSelectRow : function(rowid,e){
               var slipStatus=$(this).jqGrid("getRowData", rowid).slipStatus;
               if(slipStatus=="승인"){
                $("#slipGrid").setColProp("expenseReport", { editable: false });
                $("#slipGrid").setColProp("slipType", { editable: false});
               }
            }
...
/*    function saveJournalDetail2() {
   ...
   //일괄 저장하기
	...
	function slipInsertFunc() {
		...
		emptySlip.slipNo = "(BRC-01_05/DPT-01)SLIP" + slipToDate + "-00"
				+ nextSeq;
		emptySlip.deptCode = deptCode;
		emptySlip.reportingEmpName = empName;
		emptySlip.reportingEmpCode = empCode;
		emptySlip.slipStatus = "미결";
		emptySlip.status = "insert";
		...
		$.each(allSlipRows, function(index, val) {
			...
			emptySlip.slipNo = "(BRC-01_05/DPT-01)SLIP" + slipToDate + "-00"
					+ nextSeq;
			if (nextSeq > 9)
				emptySlip.slipNo = "(BRC-01_05/DPT-01)SLIP" + slipToDate + "-0"
						+ nextSeq;
		});
		emptySlip.reportingDate = $("#to").val();
	...
	function journalInsertFunc() {
		saveJournalDetail(slipjour["slip"], slipjour["jour"]);
		if (checkapprovalState != '승인') {
			$("#journalGrid").setColProp("rightCreditPrice", {
				editable : false
			});
			$("#journalGrid").setColProp("leftDebtorPrice", {
				editable : false
			});
			...
			//alert(slipRow);
			//alert(slipList[slipRow - 1]);
			//alert(slipList[slipRow - 1].slipNo);
				...
				.jqGrid(
						{
							...
							onSelectRow : function(rowid) {
								var codeNo = $(this)
										.jqGrid("getRowData", rowid).accountInnerCode;
...
$.jgrid.gridUnload("#codeGrid");
$("#codeGrid").jqGrid({
							url : "${pageContext.request.contextPath}/base/codeList.do?method=findDetailCodeList&code="
									+ detailCodeNo,
							datatype : "json",
							jsonReader : {
								root : "detailCodeList"
							...
							onSelectRow : function(id) {
								...
								if (detailCode != "DEC03") {
									$(grid)
											.setCell(rowid, iCol + 1,
													detailName);
								} else {
									$("#journalGrid").jqGrid('setCell', rowid,
											'descName, ""', 'editable-cell');
								}
	...
	function slipDeleteFunc() {
		...
		if (slipNo == null) {
			...
			$("#slipGrid").jqGrid('setRowData', slipRow, {
				status : "delete"
			});
	...
	function journalDeleteFunc() {
		var selectDetailRow = $("#journalGrid")
		...
		if (selectDetailRow == null) {
			...
			$("#journalGrid").jqGrid('setRowData', selectDetailRow, {
				status : "delete"
			});
			$("#slipGrid").jqGrid('setRowData', slipRow, {
				status : "update"
	...
	function showBalanceSheet() {
		...
		$.each(codeList, function(index, element) {
			var summary = new Summary(element.accountName,
					element.leftDebtorPrice, element.rightCreditPrice);
	...
	function showDebitGrid(debitTotalPrice, debitList) {
		...
		$("#debitGrid").jqGrid("footerData", "set", {
			accountName : "합계",
			leftDebtorPrice : debitTotalPrice
		});
	}
	...
	function showCreditGrid(creditTotalPrice, codeList) {
		...
		$("#creditGrid").jqGrid("footerData", "set", {
			accountName : "합계",
			rightCreditPrice : creditTotalPrice
		});
	}
						...
						.dialog(
								{
									...
									buttons : {
										ok : function() {
											$(location)
													.attr("href",
															"${pageContext.request.contextPath}/account/slip/slipform.html");
											$(this).dialog("close");
										}
									}
								});
...
<body>
	<table id="layoutSlipForm">
		...
		<tr>
			<td colspan="2">
				<!-- 분개 --> 
				<input type="button" id="journalInsertButton"
