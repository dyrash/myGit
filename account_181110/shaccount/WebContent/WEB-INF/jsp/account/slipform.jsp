...
/* 		if(serchSlipDate1==''||serchSlipDate2=='')
   ...
   function showSlipSearch(){
      ...
      $("#slipGrid").jqGrid({
         ...
         root : "slipList"
           ...
           colNames: ["전표번호","부서코드","결의일자","품의내역","전표구분","작성자코드","작성자","대차차액","승인자","승인상태","승인일자","상태","분개"],
           colModel: [
         ...
         onSelectRow: function(id){
           ...
           onCellSelect: function(rowid, iCol, cellcontent, e) {
              ...
              if(status == "select"){
                  ...
                  if(iCol == 9)
                     showCodeDialog2(this,rowid,iCol,"BRC-01","거래처");
                  }
   ...
   function showJournalDetailGrid(slipId, journalId, accountControlList){
       ...
       $.each(accountControlList, function(index, element) {
           ...
           $("<input type='text' style='height:20px; width:500px;' id='" + element.controlCode + "' value='" + journalDetailvalueArray[index] + "'>").appendTo("#journalDetailList").button();
    	   
           if(checkapprovalState != '승인'){
        	   if(element.detailType == "calendar") {
                   $("#"+element.controlCode).datepicker({dateFormat : "yy-mm-dd"});
               } else if(element.detailType == "department") {

                  $("#"+element.controlCode).click(function(event) {
                      showCodeDialog(this, null, null, element.controlCode, element.detailName);
                   });
               } else if(element.detailType == "company") {

                   $("#"+element.controlCode).click(function(event) {
                       showCodeDialog(this, null, null, element.controlCode, element.detailName);
                    });
               } else if(element.detailType == "empname") {
    	   ...
    	   saveJournalDetail(slipjour["slip"], slipjour["jour"]);
    	   $("#journalDetailList").children("input[type=text]").on("change paste keyup", function() {
   ...
   */
   ...
   colNames:['상세코드번호','상세코드이름','사용여부'],
    colModel:[
        ...
        {name:'codeUseCheck', hidden:true,width:200, align :"center",editable:false},
     ],
       width: 605,
       height: 400,
...
function showCodeDialog2(grid, rowid, iCol, detailCodeNo, detailCodeName){
	$("#codeDialog").dialog({
		title : detailCodeName ,
		"width":"650",
		"height":"450",
	...
	$.jgrid.gridUnload("#codeGrid");
	$("#codeGrid").jqGrid({
		url:"${pageContext.request.contextPath}/base/codeList.do?method=selectCustomerCodeList&code="+detailCodeNo,
		datatype: "json",
		jsonReader: { root:"customerList"},
		colNames:['거래처코드','거래처명'],
		colModel:[
	     	{name:'customerCode',width:200, align :"center",editable:false},
	     	{name:'customerName',width:200, align :"center",editable:false},
	 	],
	    width: 605,
	    height: 400,
	    caption: detailCodeName,
